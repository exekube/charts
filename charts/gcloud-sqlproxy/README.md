...
| `serviceAccountKey.create`               |     Whether to create the secret resource from `serviceAccountKey.key` value      | `true`                       |
| `serviceAccountKey.key`               |     Service account key JSON      | Must be provided if `serviceAccountKey.create` is set to `true`                     |
...
It is recommended that you generate and manage `serviceAccountKey` outside of this Helm chart. To do this, set `serviceAccountKey.create` to `false`, then generate a Kubernetes secret manually like so:

```sh
kubectl create secret generic my-release-gcloud-sqlproxy \
--namespace default \
--from-file credentials.json=$GOOGLE_CLOUD_KEYFILE_JSON
```

Note the the name of the secret, which must be: `{{ .Release.Name }}-gcloud-sqlproxy`, and the name of the key which must be `credentials.json`.
